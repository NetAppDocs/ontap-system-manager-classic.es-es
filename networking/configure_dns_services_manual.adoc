...
Las SVM utilizan la base de datos ns-switch de servicios de nombres de hosts para determinar qué servicios de nombres utilizar y en qué orden se debe buscar información sobre los hosts. Los dos servicios de nombres soportados para la base de datos hosts son `files` y. `dns`

Debe asegurarse de que `dns` es uno de los orígenes antes de crear el servidor SMB.
...
.Pasos
. Determine cuál es la configuración actual para `hosts` la base de datos de servicios de nombres.
+
...
NOTE:  `name-service dns create`El comando Vserver SERVICES realiza una validación automática de la configuración y notifica un mensaje de error si ONTAP no puede contactar con el servidor de nombres.
